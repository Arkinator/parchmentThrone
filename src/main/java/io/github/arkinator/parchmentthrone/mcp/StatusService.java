...
/*    return vectorStore.similaritySearch("Get status for nation " + nationName).stream().findFirst().map(result -> {
  ...
  public void updateNationStatus(@ToolParam(description = "Name of the nation") String nationName,
    @ToolParam(description = "Basic status of the nation") String nationStatus) {
/*    log.info("Updating basic status for nation {} to \n{}", nationName, nationStatus);
  ...
  @Tool(description = "Updates the current game data.")
  public void updateGameData(@ToolParam(description = "Current game data") GameDataDto gameData) {
