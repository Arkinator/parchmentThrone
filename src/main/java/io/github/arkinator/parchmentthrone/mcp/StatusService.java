...
//    return basicStatus;
  ...
  public void updateNationStatus(@ToolParam(description = "Name of the nation") String nationName,
    @ToolParam(description = "Basic status of the nation. Use the JSON format that has been provided") String nationStatus) {
    log.info("Updating basic status for nation {} to \n{}", nationName, nationStatus);
    vectorStore.add(new JsonReader(new ByteArrayResource(nationStatus.getBytes(StandardCharsets.UTF_8))).get());
//    log.info("Updating basic status for nation {} to: \n{}", nationName, nationStatus);
//    basicStatus = nationStatus;
  ...
  @Tool(description = "Updates the game data, which includes nation status, money, political power, and mental energy.")
